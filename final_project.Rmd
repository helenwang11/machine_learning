...
setwd("~/Documents/datascience/MachineLearning/")
library(parallel)
library(doParallel)
library(caret)
```
...
Train a random forest classifier using all the variables except "classe" as predictors.
...
table(training_pred,training_data$classe)
```

This gives us an accuracy close to 100% with the training data.
